...
class PostStatusBroadcaster_AdminPageFramework_Form_View___Script_RepeatableField extends PostStatusBroadcaster_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updatePostStatusBroadcaster_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.post-status-broadcaster-repeatable-field-buttons' )
            .filter( function() {                                       
                return $( this ).closest( '.post-status-broadcaster-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            });       
...
// @todo event.preventDefault();        
            ...
            $( this ).addPostStatusBroadcaster_AdminPageFrameworkRepeatableField();
...
// @todo event.preventDefault();        
            $( this ).removePostStatusBroadcaster_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.post-status-broadcaster-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.post-status-broadcaster-field' )
            .filter( function() {                 
                return $( this ).closest( '.post-status-broadcaster-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            })        
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addPostStatusBroadcaster_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addPostStatusBroadcaster_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.post-status-broadcaster-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.post-status-broadcaster-fields' );
        ...
        if ( ! _aOptions ) {     
            var _nodeButtonContainer = nodeFieldContainer.find( '.post-status-broadcaster-repeatable-field-buttons' )
                .filter( function() {                 
                    return $( this ).closest( '.post-status-broadcaster-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.post-status-broadcaster-field' ) 
                                    .filter( function() {                 
                                        return $( this ).closest( '.post-status-broadcaster-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.post-status-broadcaster-repeatable-field-buttons' )
                                    .filter( function() {                 
                                        return $( this ).closest( '.post-status-broadcaster-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    }); 
         ...
         */
        nodeNewField.updatePostStatusBroadcaster_AdminPageFrameworkRepeatableFields();
        ...
        $( nodeNewField ).find( '.post-status-broadcaster-field' ).addBack().trigger( 
            'post-status-broadcaster_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.post-status-broadcaster-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {                      
                return $( this ).closest( '.post-status-broadcaster-fields' ).attr( 'id' ) === _sFieldsContainerID;                
            })
        ...
        nodeFieldsContainer.children( '.post-status-broadcaster-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.post-status-broadcaster-field,.post-status-broadcaster-fields,.post-status-broadcaster-fieldset' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removePostStatusBroadcaster_AdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.post-status-broadcaster-field' );
        var nodeFieldsContainer = $( this ).closest( '.post-status-broadcaster-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
