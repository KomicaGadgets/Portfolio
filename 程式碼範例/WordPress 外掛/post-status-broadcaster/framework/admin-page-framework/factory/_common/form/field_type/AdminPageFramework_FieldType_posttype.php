...
class PostStatusBroadcaster_AdminPageFramework_FieldType_checkbox extends PostStatusBroadcaster_AdminPageFramework_FieldType {
...
jQuery( document ).ready( function(){
    ...
    jQuery( '{$_sClassSelectorSelectAll}' ).each( function(){
        jQuery( this ).before( '<div class=\"select_all_button_container\" onclick=\"jQuery( this ).selectAllPostStatusBroadcaster_AdminPageFrameworkCheckboxes(); return false;\"><a class=\"select_all_button button button-small\">' + jQuery( this ).data( 'select_all_button' ) + '</a></div>' );
    });            
    jQuery( '{$_sClassSelectorSelectNone}' ).each( function(){
        jQuery( this ).before( '<div class=\"select_none_button_container\" onclick=\"jQuery( this ).deselectAllPostStatusBroadcaster_AdminPageFrameworkCheckboxes(); return false;\"><a class=\"select_all_button button button-small\">' + jQuery( this ).data( 'select_none_button' ) + '</a></div>' );
    });
});
JAVASCRIPTS;
    ...
    private function _getSelectButtonClassSelectors(array $aFieldTypeSlugs, $sDataAttribute = 'select_all_button') {
        ...
        foreach ($aFieldTypeSlugs as $_sSlug) {
            ...
            $_aClassSelectors[] = '.post-status-broadcaster-checkbox-container-' . $_sSlug . "[data-{$sDataAttribute}]";
        }
        return implode(',', $_aClassSelectors);
    }
    protected function getStyles() {
        return ".select_all_button_container, .select_none_button_container{display: inline-block;margin-bottom: 0.4em;}.post-status-broadcaster-checkbox-label {margin-top: 0.1em;}.post-status-broadcaster-field input[type='checkbox' ] {margin-right: 0.5em;} .post-status-broadcaster-field-checkbox .post-status-broadcaster-input-label-container {padding-right: 1em;}.post-status-broadcaster-field-checkbox .post-status-broadcaster-input-label-string{display: inline; }";
    }
    ...
    class PostStatusBroadcaster_AdminPageFramework_FieldType_posttype extends PostStatusBroadcaster_AdminPageFramework_FieldType_checkbox {
        ...
        protected function getStyles() {
            ...
            return $_sParentStyles . ".post-status-broadcaster-field input[type='checkbox'] {margin-right: 0.5em;} .post-status-broadcaster-field-posttype .post-status-broadcaster-input-label-container {padding-right: 1em;}";
        }
        protected function getField($aField) {
