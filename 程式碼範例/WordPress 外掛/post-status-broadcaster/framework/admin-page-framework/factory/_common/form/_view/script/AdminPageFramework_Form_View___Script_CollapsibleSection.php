...
class PostStatusBroadcaster_AdminPageFramework_Form_View___Script_CollapsibleSection extends PostStatusBroadcaster_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
    ...
    jQuery( document ).ready( function() {
        jQuery( this ).initializePostStatusBroadcaster_AdminPageFrameworkCollapsibleSections();
    });              
     ...
     * @since    3.7.0
     */
    jQuery( document ).bind( 'post-status-broadcaster_saved_widget', function( event, oWidget ){
        jQuery( oWidget ).initializePostStatusBroadcaster_AdminPageFrameworkCollapsibleSections();
    });    
    
    $.fn.initializePostStatusBroadcaster_AdminPageFrameworkCollapsibleSections = function() {
        
        // Expand collapsible sections that are set not to collapse by default 
        jQuery( this ).find( '.post-status-broadcaster-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.post-status-broadcaster-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.post-status-broadcaster-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.post-status-broadcaster-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );
            
        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        jQuery( this ).find( '.post-status-broadcaster-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.post-status-broadcaster-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.post-status-broadcaster-collapsible-sections-title, .post-status-broadcaster-collapsible-section-title' )
            .enablePostStatusBroadcaster_AdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.post-status-broadcaster-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'post-status-broadcaster-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';            
                _oButton.find( '.post-status-broadcaster-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.post-status-broadcaster-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.post-status-broadcaster-section' )[ _sTopOrBottom ]( _oButton );
                }                
                ...
                _oButton.click( function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.post-status-broadcaster-sectionset' ).siblings().andSelf().find( '> .post-status-broadcaster-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.post-status-broadcaster-collapsible-toggle-all-button-container' ).andSelf();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .post-status-broadcaster-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.post-status-broadcaster-sections' ).children( '.post-status-broadcaster-section' ).children( '.post-status-broadcaster-section-table' ).children( 'caption' ).children( '.post-status-broadcaster-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
     ...
     */
    $.fn.enablePostStatusBroadcaster_AdminPageFrameworkCollapsibleButton = function() {
        ...
        function _isFieldElement( nodeTarget ) {
            ...
            if ( jQuery( nodeTarget ).hasClass( 'post-status-broadcaster-collapsible-button' ) ) {
                return false;
            }
        ...
        jQuery( this ).click( function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'post-status-broadcaster-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.post-status-broadcaster-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .post-status-broadcaster-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.post-status-broadcaster-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='post-status-broadcaster-collapsible-toggle-all-button-container'>" . "<span class='post-status-broadcaster-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
    }
