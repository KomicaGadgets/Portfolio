...
class PostStatusBroadcaster_AdminPageFramework_FieldType_text extends PostStatusBroadcaster_AdminPageFramework_FieldType {
    public $aFieldTypeSlugs = array('text', 'password', 'date', 'datetime', 'datetime-local', 'email', 'month', 'search', 'tel', 'url', 'week',);
    protected $aDefaultKeys = array();
    protected function getStyles() {
        return ".post-status-broadcaster-field.post-status-broadcaster-field-text > .post-status-broadcaster-input-label-container {vertical-align: middle; }.post-status-broadcaster-field.post-status-broadcaster-field-text > .post-status-broadcaster-input-label-container.post-status-broadcaster-field-text-multiple-labels {display: block;}";
    }
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField) {
        ...
        $_sClassSelector = $_bIsArray ? 'post-status-broadcaster-field-text-multiple-labels' : '';
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $sKey, $aField['label']), "<div class='post-status-broadcaster-input-label-container {$_sClassSelector}'>", "<label for='" . $_aInputAttributes['id'] . "'>", $this->getElementByLabel($aField['before_input'], $sKey, $aField['label']), $_sLabel ? "<span " . $this->getLabelContainerAttributes($aField, 'post-status-broadcaster-input-label-string') . ">" . $_sLabel . "</span>" : '', "<input " . $this->getAttributes($_aInputAttributes) . " />", $this->getElementByLabel($aField['after_input'], $sKey, $aField['label']), "</label>", "</div>", $this->getElementByLabel($aField['after_label'], $sKey, $aField['label']),);
        return implode('', $_aOutput);
    }
    }
    class PostStatusBroadcaster_AdminPageFramework_FieldType_file extends PostStatusBroadcaster_AdminPageFramework_FieldType_text {
