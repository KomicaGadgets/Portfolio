...
class PostStatusBroadcaster_AdminPageFramework_Form_View__Resource__Head extends PostStatusBroadcaster_AdminPageFramework_FrameworkUtility {
    public $oForm;
    ...
    public function _replyToInsertRequiredInternalScripts() {
        ...
        echo "<script type='text/javascript' class='post-status-broadcaster-form-script-required-in-head'>" . '/* <![CDATA[ */ ' . $this->_getScripts_RequiredInHead() . ' /* ]]> */' . "</script>";
    }
    private function _getScripts_RequiredInHead() {
        return 'document.write( "<style class=\'post-status-broadcaster-js-embedded-internal-style\'>' . str_replace('\\n', '', esc_js($this->_getInternalCSS())) . '</style>" );';
    }
    private function _getInternalCSS() {
        $_oLoadingCSS = new PostStatusBroadcaster_AdminPageFramework_Form_View___CSS_Loading;
    ...
    private function _getScriptElementConcealerCSSRules() {
        return ".post-status-broadcaster-form-js-on {visibility: hidden;}.widget .post-status-broadcaster-form-js-on { visibility: visible; }";
    }
    }
